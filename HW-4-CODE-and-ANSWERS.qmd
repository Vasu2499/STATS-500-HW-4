...
# # I created both vectors manually to avoid errors related to data loading/type mismatch 
...
```{r}
# Scale-Location (Square root of standardized residuals vs. fitted values)
...
for (b in 1:B) {
  epsilon <- rnorm(n, mean = 0, sd = sqrt(0.75)) # Normally distributed residuals
  y <- 1 + 5 * x + epsilon # Generate y
  model <- lm(y ~ x) # Fit linear model
  results[b] <- coef(model)[2] # Store the slope estimate
}

# Calculate the coverage of the 80% CI
lower_bound <- quantile(results, 0.1)
upper_bound <- quantile(results, 0.9)
coverage <- mean((lower_bound <= 5) & (upper_bound >= 5)) # True slope is 5
coverage
...
for (b in 1:B) {
  epsilon <- rlnorm(n, meanlog = 0, sdlog = 0.5) - 1 # Log-normal residuals, shifted to have mean zero
  y <- 1 + 5 * x + epsilon
  model <- lm(y ~ x)
...
coverage_log_normal <- mean((lower_bound_log_normal <= 5) & (upper_bound_log_normal >= 5))
...
for (b in 1:B) {
  ...
  # Introduce outliers
  if (b %% 100 == 0) { # Introduce outliers every 100 simulations
    y[c(1, n)] <- y[c(1, n)] + c(15, -15) # Adding large positive and negative outliers
  }
...
coverage_outliers <- mean((lower_bound_outliers <= 5) & (upper_bound_outliers >= 5))
...
hist(Epsilon.skew[, 1], main = "Histogram of Error Terms (Iteration 1)",
     xlab = "Error Terms", breaks = 40, col = "lightblue")

# Normal Quantile Plot
qqnorm(Epsilon.skew[, 1], main = "Normal Q-Q Plot of Error Terms (Iteration 1)")
...
hist(b1.skew, probability = TRUE, main = "Distribution of Sample Slopes",
...
hist(se.skew, main = "Distribution of Standard Errors (Homoskedasticity)",
     xlab = "Standard Error", col = "coral", breaks = 40)

# Histogram for se.skew.hc
hist(se.skew.hc, main = "Distribution of Standard Errors (Heteroskedasticity Consistent)",
...
```{r}
# True population slope (replace with the actual value if different)
true_slope = beta1
...
coverage_hom = mean(lower_bound_hom <= true_slope & upper_bound_hom >= true_slope)
coverage_het = mean(lower_bound_het <= true_slope & upper_bound_het >= true_slope)
...
hist(Epsilon.het[, 1], main = "Histogram of Error Terms (Iteration 1)", xlab = "Error Terms", breaks = 40, col = "lightblue")
...
# Histogram of estimated slopes
hist(b1.het, probability = TRUE, main = "Distribution of Sample Slopes", xlab = "Estimated Slope Coefficient", col = "lightgreen", breaks = 40)

# Overlay normal density
curve(dnorm(x, mean = mean(b1.het), sd = sd(b1.het)), add = TRUE, col = "red")

# Histogram for se.het
hist(se.het, main = "Distribution of Standard Errors (Homoskedasticity)", xlab = "Standard Error", col = "lightcoral", breaks = 40)

# Histogram for se.het.hc
hist(se.het.hc, main = "Distribution of Standard Errors (Heteroskedasticity Consistent)", xlab = "Standard Error", col = "lightyellow", breaks = 40)
...
# Coverage calculation
coverage_hom_het = mean(lower_bound_hom_het <= true_slope & upper_bound_hom_het >= true_slope)
coverage_het_hc = mean(lower_bound_het_hc <= true_slope & upper_bound_het_hc >= true_slope)
...
plot(x, Epsilon.het[, 1], main = "Scatter Plot of x vs. Epsilon (Iteration 1)", xlab = "x", ylab = "Epsilon", col = "blue", pch = 19, cex=0.8)
...
```{r}
hist(b1.het, probability = TRUE, main = "Distribution of Sample Slopes", xlab = "Estimated Slope Coefficient", col = "lightgreen", breaks = 30)
...
hist(se.het, main = "Distribution of Standard Errors (Homoskedasticity)", xlab = "Standard Error", col = "coral", breaks = 40)

hist(se.het.hc, main = "Distribution of Standard Errors (Heteroskedasticity Consistent)", xlab = "Standard Error", col = "yellow", breaks = 40)
